  ...
  try {
    ...
    if (!favoresSnapshot.empty) {
      ...
      for (const doc of favoresSnapshot.docs) {
        const data = doc.data();
        logger.info(`🗑️ Eliminando favor finalizado: ${doc.id} - "${data.titulo}" (finalizado el ${data.fechaFinalizacion?.toDate().toISOString()})`);
        ...
        try {
          ...
          if (!reportesSnapshot.empty) {
            logger.info(`📝 Eliminando ${reportesSnapshot.size} reportes asociados al favor ${doc.id}`);
        ...
        if (count >= 450) { // 450 para dejar margen a los reportes
          logger.warn("⚠️ Alcanzado límite de operaciones en batch, ejecutando...");
          await batch.commit();
      ...
      logger.info(`🎉 Limpieza completada. Total eliminados: ${totalEliminados} favores finalizados`);
    } else {
