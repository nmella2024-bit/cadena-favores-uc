 ...
 * Optimized for efficiency, security, and Vite/Rollup compatibility.
 */
 ...
 */
const generateQuiz = async (topic, config = {}) => {
    try {
 ...
 */
const gradeAnswer = async (question, userAnswer) => {
    try {
        const response = await fetch(API_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
 ...
 */
const generateWeaknessQuiz = async (weakTopics) => {
...
// Export object with named export for compatibility
const studyAI = {
    generateQuiz,
    gradeAnswer,
